...
Eigen::Matrix<float,9,1> RBDsolve::Verify(Eigen::Matrix<float,9,1> states){
    ...
    states_dot2(0) = std::sin(t) + std::cos(y1) + std::cos(y2); // dy1/dt
    states_dot2(1) = std::cos(t) + std::sin(y2) + std::cos(y1); // dy2/dt
    states_dot2(2) = 1.0f;                                       // dt/dt

    return states_dot2;
}
Eigen::Matrix<float,9,1> RBDsolve::Equations(Eigen::Matrix<float,9,1> states){
    // Unpack states (this makes Equations() a pure function of its input;
    // no need to call setState() separately before each RK4 stage)
    ...
    F_g(0) = -m * g * sin(euler[1]) + F_g0[0];
    F_g(1) =  m * g * cos(euler[1]) * sin(euler[0]) + F_g0[1];
    F_g(2) =  m * g * cos(euler[1]) * cos(euler[0]) + F_g0[2];
    ...
    // Build states_dot (9x1) - EACH ELEMENT INDIVIDUALLY
...
void RBDsolve::setState(const Eigen::Matrix<float,9,1>& states) {
    v = states.segment<3>(0);
    omega = states.segment<3>(3);
    euler = states.segment<3>(6);
}